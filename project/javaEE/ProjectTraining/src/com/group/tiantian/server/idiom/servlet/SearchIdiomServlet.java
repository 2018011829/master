...
/**
 * Servlet implementation class SearchIdiomServlet
 */
@WebServlet("/SearchIdiomServlet")
public class SearchIdiomServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SearchIdiomServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(userName!=null && !userName.equals("")) {
			...
			Page<Idiom> pageInfo=idiomTypeService.idiomListByPage(pageNum, pageSize,searchInfo);
			request.setAttribute("page", pageInfo);
			request.setAttribute("searchInfo", searchInfo);
			request.setAttribute("userName", userName);
			request.getRequestDispatcher("showSearchIdiom.jsp").forward(request, response);
		}else {
